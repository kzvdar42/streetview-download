...
from core.pans_download import get_tiles_info, get_grid_panoids
...
def download_panorama_v5(panoid, zoom=5, max_retry=3):
    ...
    while (is_empty).any() and n_retry < max_retry:
        if n_retry > 0:
            tqdm.write("Connection error. Trying again in 1 second.")
            time.sleep(1)
...
def main(args, writer=None):
    ...
    for i, pos_box in enumerate(pos_boxes, 1):
        ...
        pan_infos = [indexed_pan_ids.add(p['panoid']) or p for p in pan_infos if p['panoid'] not in indexed_pan_ids]
        # Filter by min year
